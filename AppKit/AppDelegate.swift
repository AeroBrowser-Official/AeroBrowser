...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, ObservableObject {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !UserDefaults.standard.bool(forKey: "hasShownIntro") {
            AppDelegate.shared = self // ✅ make sure shared is set
            let intro = IntroOverlayController()

            NotificationCenter.default.addObserver(forName: .didFinishIntroAnimation, object: nil, queue: .main) { _ in
                // 👇 Wait a bit for fade-out, then show main window
            ...
            AppDelegate.shared = self // ✅ required anyway
            createWindow()
        ...
        @objc func openAboutWindow() {
            if let keyWindow = NSApplication.shared.keyWindow {
                let windowNumber = keyWindow.windowNumber
                if let target = self.service.browsers[windowNumber] {
                    target.selectedSettingsCategory = .About
                    target.openSettings()
                }
            }
        }
